...
func main() {
	...
	if !checkAudioPlayer() {
		fmt.Println("⚠️  No audio player detected!")
		fmt.Println()
		fmt.Println("For Alpine Linux, install ffmpeg:")
		fmt.Println("  apk add ffmpeg")
		fmt.Println()
		fmt.Println("Or install mpg123:")
		fmt.Println("  apk add mpg123")
		fmt.Println()
		os.Exit(1)
	}
...
func checkAudioPlayer() bool {
	players := []string{"ffplay", "mpg123", "afplay", "mplayer", "play"}
...
func playRound(game *Game) bool {
	...
	if err != nil {
		fmt.Printf("Warning: Audio playback issue: %v\n", err)
		fmt.Println("Continuing with quiz anyway...")
	}
...
func playAudioFile(filename string) error {
	...
	switch runtime.GOOS {
	case "linux":
		// For Alpine Linux, prefer ffplay or mpg123
		...
		} else if _, err := exec.LookPath("mplayer"); err == nil {
			playerName = "mplayer"
			cmd = exec.Command("mplayer", "-really-quiet", filename)
		} else if _, err := exec.LookPath("play"); err == nil {
			playerName = "play"
			cmd = exec.Command("play", filename)
		} else {
			return fmt.Errorf("no audio player found. Please install: apk add ffmpeg (or mpg123)")
		}
	case "darwin": // macOS
		playerName = "afplay"
	...
	go func() {
		...
		if err != nil && len(output) > 0 {
			// Only print errors that aren't timeout-related
			outputStr := string(output)
			if !strings.Contains(outputStr, "Exiting") {
				fmt.Printf("Audio player (%s) message: %s\n", playerName, outputStr)
			}
		}
		done <- err
	}()
	...
	case err := <-done:
		if err != nil {
			return fmt.Errorf("playback failed with %s: %v", playerName, err)
		}
		return nil
